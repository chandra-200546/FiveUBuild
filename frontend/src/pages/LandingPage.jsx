    ...
    <div>
      ...
      <main className="mx-auto max-w-7xl px-6 py-16">
        <section className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            <h1 className="text-5xl font-semibold leading-tight">Build Full Web Apps From a Single Prompt</h1>
            <p className="mt-5 text-slate-300">FiveUBuild turns your ideas into complete HTML, CSS, and JavaScript apps with live preview, editable code, and one-click download.</p>
            <div className="mt-8 flex gap-3">
              <Link to="/register" className="rounded-lg bg-gradient-to-r from-blue-500 to-violet-500 px-6 py-3 font-medium">Start Building</Link>
              <a href="#how" className="rounded-lg border border-white/20 px-6 py-3">View Demo</a>
            </div>
          </div>
          <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity }} className="glass rounded-2xl p-4">
        ...
        <section className="mt-20 grid gap-4 md:grid-cols-4">{features.map((f) => <div className="glass rounded-xl p-4 text-sm" key={f}>{f}</div>)}</section>
        ...
        <section id="how" className="mt-20 grid gap-4 md:grid-cols-4">{["Describe your idea", "AI generates code", "Preview and edit", "Download or save"].map((s, i) => <div className="glass rounded-xl p-4" key={s}><p className="text-xs text-blue-300">Step {i + 1}</p><p className="font-medium">{s}</p></div>)}</section>
        <section className="mt-20 grid gap-4 md:grid-cols-3">{showcases.map((s) => <div key={s} className="glass rounded-xl p-4">{s}</div>)}</section>
        <section className="mt-20 grid gap-4 md:grid-cols-3">{["Free Plan", "Pro Plan", "Founder Plan"].map((p) => <div key={p} className="glass rounded-xl p-6"><h3 className="text-xl font-semibold">{p}</h3></div>)}</section>
